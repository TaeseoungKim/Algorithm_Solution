import unittest
# PreOrder traverse(선위 순회):
# 1. root 방문
# 2. 왼쪽 자식 노드 방문
# 3. 오른쪽 자식 노드 방문

# InOrder traverse(중위 순회):
# 이진트리의 경우, 중위 순회를 하면 정렬이 되어 출력된다.
# 1. 왼쪽 자식 노드 방문
# 2. 부모 노드 방문
# 3. 오른쪽 자식 노드 방문

# PostOrder traverse(중위 순회):
# 1. 왼쪽 자식 노드 방문
# 2. 오른쪽 자식 노드 방문
# 3. root 노드 방문
    ...
    def remove(self, item):
        ...
        if self.head.val == item:
            ...
            # 4) Node to be removed has two children.
                ...
                self.head.val = self.__most_left_val_from_right_node(
                    self.head.right).val
...
class binary_tree_test(unittest.TestCase):
    def test(self):
        ...
        self.assertEqual(
            bt_left_remove_test_2_left.preorder_list, [60, 40, 70])
        ...
        self.assertEqual(
            bt_left_remove_test_2_right.preorder_list, [60, 55, 70])
        ...
        self.assertEqual(
            bt_right_remove_test_2_left.preorder_list, [60, 50, 65])
        ...
        self.assertEqual(
            bt_right_remove_test_2_right.preorder_list, [60, 50, 75])
        ...
        self.assertEqual(bt_left_remove_test_3.preorder_list,
                         [60, 52, 40, 55, 70])
